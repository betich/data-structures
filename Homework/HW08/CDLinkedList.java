public class CDLinkedList {
	...
	public void sort() throws Exception { // any sorting will do BUT you must use only 'this' list. No array or any other data structures allowed!!!
		...
		DListNode current = header.nextNode.nextNode;

		while (current != header) {
			int key = current.data;
			DListNode prev = current.previousNode;

			while (prev != header && prev.data > key) {
				prev.nextNode.data = prev.data;
				prev = prev.previousNode;
			}

			prev.nextNode.data = key;
