...
public class CDLinkedList {
	...
	public void swapRange(Iterator s1, Iterator f1, Iterator s2, Iterator f2) {
		// check if any of the iterators are null or not of type DListIterator
		if (s1 == null || !(s1 instanceof DListIterator) || f1 == null || !(f1 instanceof DListIterator) ||
				s2 == null || !(s2 instanceof DListIterator) || f2 == null || !(f2 instanceof DListIterator)) {
			return;
		}

		if (((DListIterator) f1).currentNode.nextNode == ((DListIterator) s2).currentNode) {
			DListIterator t1 = new DListIterator(((DListIterator) s1).currentNode.previousNode);
			DListIterator t4 = new DListIterator(((DListIterator) f2).currentNode.nextNode);

			((DListIterator) s2).currentNode.previousNode = t1.currentNode;
			t1.currentNode.nextNode = ((DListIterator) s2).currentNode;

			((DListIterator) f1).currentNode.nextNode = t4.currentNode;
			t4.currentNode.previousNode = ((DListIterator) s1).currentNode;

			((DListIterator) s1).currentNode.previousNode = ((DListIterator) f2).currentNode;
			((DListIterator) f2).currentNode.nextNode = ((DListIterator) s1).currentNode;

		} else {
			DListIterator t1 = new DListIterator(((DListIterator) s1).currentNode.previousNode);
			DListIterator t2 = new DListIterator(((DListIterator) f1).currentNode.nextNode);
			DListIterator t3 = new DListIterator(((DListIterator) s2).currentNode.previousNode);
			DListIterator t4 = new DListIterator(((DListIterator) f2).currentNode.nextNode);

			t1.currentNode.nextNode = ((DListIterator) s2).currentNode;
			((DListIterator) s2).currentNode.previousNode = t1.currentNode;

			t2.currentNode.previousNode = ((DListIterator) f2).currentNode;
			((DListIterator) f2).currentNode.nextNode = t2.currentNode;

			t3.currentNode.nextNode = ((DListIterator) s1).currentNode;
			((DListIterator) s1).currentNode.previousNode = t3.currentNode;

			t4.currentNode.previousNode = ((DListIterator) f1).currentNode;
			((DListIterator) f1).currentNode.nextNode = t4.currentNode;
		}
	}
